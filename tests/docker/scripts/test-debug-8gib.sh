#!/bin/bash
# Debug version of 8GiB SSF test with smaller size
set -euo pipefail

echo "=========================================="
echo "BenchFS IOR Debug Test: Mini 8GiB SSF"
echo "Nodes: 4, Size: 512MB per rank (64MB x 8)"
echo "=========================================="
...
echo
echo "=========================================="
echo "Running Mini 8GiB SSF Test"
echo "Size: 512MB per rank (64MB x 8 segments)"
echo "Total: 2GB across 4 ranks"
echo "=========================================="
...
mpirun \
    ...
    ${IOR_BIN} \
        ...
        -w -r \
        -t 2m -b 64m -s 8 \
        -o /test_mini_8gib_ssf \
        -O summaryFormat=JSON \
        > ${RESULTS_DIR}/ior_mini_output.txt 2>&1

IOR_EXIT_CODE=$?

echo
echo "IOR Output:"
cat ${RESULTS_DIR}/ior_mini_output.txt
